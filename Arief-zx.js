...
const { help, bahasa, donasi, limitend, limitcount, bottt } = require('./Arief-zx/help')
...
prefix = "z"
name = "Arief-zx Bot"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN ARIEF BOT"
rmenu = "_*Created By Arief-zx And Verifcated By WhatsApp*_"
botinfo = "_*Invite Bot Donasi Gan Ketik zowner Untuk Donasi*_"
limitt = 50
memberLimit = 1
ban = []
premium = ["62895600555028@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('connecting', () => {
		start('2', 'Sabar Lah Ngab')
	})
	frhan.on('open', () => {
		success('2', 'Connected To Arief-zx Bot')
	})
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}* Ngab`
				...
				teks = `Sayonara @${num.split('@')[0]} Nitip Gorengan Bro`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["62895600555028@s.whatsapp.net"]
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen Arief-zx')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case 's':
						...
						if (isLimit(sender)) return reply(limitend(pushname2))
						reply(mess.wait)
						await ffmpeg(sticker, text, FarhanGans, rmenu)
            ...
            break  
				...
				case 'owner':
                 ...
                 frhan.sendMessage(from, '_Ini Owner Arief-zx Yang Paling Gans_',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('62895600555028@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 Arief-zx 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 Arief-zx 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat Arief-zx Bot\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							frhan.sendMessage(_.jid, bc, image, {caption: `*[ Arief-zx Bot ]*\n\n${body.slice(4)}`})
						}
						reply('Suksess broadcast')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「Arief-zx Broadcast」*\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[Arief-zx]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
